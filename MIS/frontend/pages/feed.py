...
async def get_top_meetings() -> list[Meeting]:
    ...
    sorted_meetings = sorted(meetings, key=lambda meeting: meeting.id, reverse=True)[:10]
...
if latest_meetings:
    ...
    for meeting in latest_meetings:
        ...
        with col2:
            st.button(
                "Transcript", on_click=transcript_btn_click, kwargs={"index": meeting.id}, key=meeting.id
            )
...
with st.sidebar:
    ...
    with st.expander("Chats", True):
        ...
        summary_button = st.button("Chat Summary", key="chat_summary", disabled=True)
        if chats:
            st.divider()
            for chat in chats:
