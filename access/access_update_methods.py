...
async def update_table_from_model(obj: BaseModelSubClass) -> BaseModelSubClass:
    ...
    primary_key_column = obj.__primarykey__  # Assuming the primary key column is defined in the model
...
async def update_table(
    ...
    """
    assert issubclass(model, DatabaseModel), f"Object is not a Database Pydantic model: {model}"
    ...
    parameters = tuple(
        dumps(value) if isinstance(value, dict) else value for value in updates.values()
    ) + tuple(
        dumps(value) if isinstance(value, dict) else value for value in conditions.values()
    )
